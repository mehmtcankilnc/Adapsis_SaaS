...
const formatActivityDateTime = (dateStr: string, locale: string) =>
  new Intl.DateTimeFormat(locale, { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }).format(new Date(dateStr))

const formatTaskDueDate = (dateStr: string, locale: string) =>
  new Intl.DateTimeFormat(locale, { day: '2-digit', month: 'short', year: 'numeric' }).format(new Date(dateStr))
...
export default function DashboardClient({
  ...
  async function handleCompleteTask(taskId: string) {
    ...
    if (res.success) {
      ...
      toast.success(t("sales.dashboard.taskCompletedToast"))
    } else {
      toast.error(t("sales.dashboard.taskUpdateFailedToast"), { description: res.error })
    }
  }
  ...
  const metrics = useMemo(() => {
    ...
    filteredQuotes.forEach((q) => {
      ...
      const pName = q.products?.name || t("sales.dashboard.unknownProduct");
      productCounts[pName] = (productCounts[pName] || 0) + 1;
    });

    let popularProduct = t("sales.dashboard.noData");
    let maxCount = 0;
    for (const [name, count] of Object.entries(productCounts)) {
      if (count > maxCount && name !== t("sales.dashboard.unknownProduct")) {
        maxCount = count;
        popularProduct = name;
      }
    }
  ...
  const topCustomers = useMemo(() => {
    ...
    filteredQuotes.forEach((q) => {
      ...
      const name = q.customers?.company_name || t("sales.dashboard.unknownCustomer");
  ...
  const leaderboard = useMemo(() => {
    ...
    filteredQuotes.forEach((q) => {
      ...
      const name = q.creator_name || t("sales.dashboard.unknownRep");
    ...
    if (timeFilter === '1year') {
      const last12Months = Array.from({length: 12}).map((_, i) => {
        ...
        return {
          ...
          name: d.toLocaleDateString(locale, { month: 'short', year: 'numeric' }),
          count: 0
        };
      });
      filteredQuotes.forEach((q) => {
  ...
  const pieData = useMemo(() => {
    ...
    return [
      { name: t("sales.dashboard.pieStatus.pending"), value: counts.pending },
      { name: t("sales.dashboard.pieStatus.accepted"), value: counts.accepted },
      { name: t("sales.dashboard.pieStatus.rejected"), value: counts.rejected },
      { name: t("sales.dashboard.pieStatus.pendingAdminApproval"), value: counts.pending_admin_approval },
    ];
  }, [filteredQuotes]);
        ...
        <div className="max-w-md text-center bg-white p-10 rounded-2xl shadow-sm border border-slate-200">
          ...
          {error === 'unauthorized' && (
            <div className="bg-red-50 text-red-600 px-4 py-3 rounded-lg border border-red-100 flex items-center justify-center mb-6 text-sm font-medium">
              ...
              {t("sales.dashboard.unauthorizedMessage")}
            </div>
          )}

          <h2 className="text-xl font-bold text-slate-900 mb-2">{t("sales.dashboard.emptyState.title")}</h2>
          <p className="text-slate-500 mb-8 text-sm">
            {t("sales.dashboard.emptyState.description")}
          </p>
          {role === 'sales' ? (
            <div className="flex flex-col gap-3">
              <Link href="/sales/new-quote">
                <Button variant="primary" className="w-full h-14 text-lg">
                  {t("sales.dashboard.emptyState.createQuoteButton")}
                </Button>
              </Link>
            </div>
          ) : (
            <div className="flex flex-col gap-3">
              <Link href="/admin/products">
                <Button variant="outline" className="w-full">{t("sales.dashboard.emptyState.expandCatalogButton")}</Button>
              </Link>
              <Link href="/admin/products/new">
                <Button variant="primary" className="w-full">{t("sales.dashboard.emptyState.createFirstDraftButton")}</Button>
              </Link>
            </div>
          )}
        </div>
    ...
    <div className="min-h-screen bg-slate-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 border-b border-slate-200 pb-5 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">{t("sales.dashboard.title")}</h1>
            <p className="text-sm text-slate-500 mt-1">{t("sales.dashboard.subtitle")}</p>
          </div>
          {error === 'unauthorized' && (
          ...
          <div className="mb-8 p-4 bg-amber-50 border border-amber-200 rounded-lg flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-start gap-3">
              ...
              <p className="text-sm text-amber-800">
                <span className="font-bold">{staleQuotes.length} {t("sales.dashboard.staleQuotes.quoteCountSuffix")}</span>{' '}
                {STALE_DAYS} {t("sales.dashboard.staleQuotes.waitingText")}
                {oldestStaleDays > STALE_DAYS && (
                  <> ({t("sales.dashboard.staleQuotes.oldestPrefix")} {oldestStaleDays} {t("sales.dashboard.staleQuotes.oldestSuffix")})</>
                )}.
              </p>
            </div>
            <Link href="/sales/quotes" className="shrink-0">
              <Button variant="outline" size="sm" className="bg-white border-amber-300 text-amber-700 hover:bg-amber-100">
                {t("sales.dashboard.staleQuotes.viewQuotesButton")}
              </Button>
            </Link>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Card className="border-slate-200 shadow-sm relative overflow-hidden">
            <CardContent className="p-6 flex flex-col justify-between h-full">
              <div className="flex justify-between items-start mb-4">
                <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t("sales.dashboard.metrics.totalQuotesLabel")}</div>
              ...
              <div className="mt-2">
                ...
                <div className="text-xs text-slate-500 mt-2 font-medium">{t("sales.dashboard.metrics.totalQuotesCaption")}</div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-slate-200 shadow-sm relative overflow-hidden">
            <CardContent className="p-6 flex flex-col justify-between h-full">
              <div className="flex justify-between items-start mb-4">
                <div className="text-xs font-bold text-slate-400 uppercase tracking-wider line-clamp-1 truncate" title={t("sales.dashboard.metrics.pendingRevenueTooltip")}>{t("sales.dashboard.metrics.pendingRevenueLabel")}</div>
              ...
              <div className="mt-2">
                ...
                <div className="text-xs text-slate-500 mt-2 flex items-center font-medium"><AlertCircle className="w-4 h-4 mr-1 inline text-brand-500"/> {t("sales.dashboard.metrics.pendingRevenueCaption")}</div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-slate-200 shadow-sm">
            <CardContent className="p-6 flex flex-col justify-between h-full">
              <div className="flex justify-between items-start mb-4">
                <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t("sales.dashboard.metrics.acceptanceRateLabel")}</div>
              ...
              <div className="mt-2">
                ...
                <div className="text-xs mt-2 text-emerald-600 font-medium">{t("sales.dashboard.metrics.acceptanceRateCaption")}</div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-slate-200 shadow-sm">
            <CardContent className="p-6 flex flex-col justify-between h-full">
              <div className="flex justify-between items-start mb-4">
                <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t("sales.dashboard.metrics.popularProductLabel")}</div>
              ...
              <div className="text-xs text-slate-500 font-medium mt-auto pt-4">{t("sales.dashboard.metrics.popularProductCaption")}</div>
            </CardContent>
          </Card>
        </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-base font-bold text-slate-800 flex items-center">
                  <Target className="w-5 h-5 mr-2 text-brand-500" /> {t("sales.dashboard.myQuota.title")}
                </h3>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 rounded-lg p-4">
                  <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">{t("sales.dashboard.myQuota.commissionLabel")}</div>
                ...
                <div className="bg-slate-50 rounded-lg p-4">
                  <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">{t("sales.dashboard.myQuota.remainingLabel")}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-2 border-slate-200 shadow-sm">
            <CardContent className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-base font-bold text-slate-800 flex items-center">
                  <TrendingUp className="w-5 h-5 mr-2 text-brand-500"/> {t("sales.dashboard.charts.quoteActivityTitle")}
                </h3>
                <select
                  value={timeFilter}
                ...
                >
                  <option value="7days">{t("sales.dashboard.filters.last7Days")}</option>
                  <option value="1month">{t("sales.dashboard.filters.last1Month")}</option>
                  <option value="1year">{t("sales.dashboard.filters.last1Year")}</option>
                  <option value="all">{t("sales.dashboard.filters.allTime")}</option>
                </select>
              </div>
              <div className="h-[300px] w-full">
                <ResponsiveContainer width="100%" height="100%" key={timeFilter}>
                  <LineChart data={lineData} margin={{ top: 5, right: 20, bottom: 5, left: -20 }}>
                    ...
                    />
                    <Line type="monotone" dataKey="count" name={t("sales.dashboard.charts.quoteCountSeries")} stroke="#2563eb" strokeWidth={4} dot={{r: 4, strokeWidth: 2, fill: '#fff'}} activeDot={{r: 7, fill: '#2563eb', stroke: '#fff', strokeWidth: 2}} animationDuration={500} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
          ...
          <Card className="border-slate-200 shadow-sm">
            <CardContent className="p-6">
              <h3 className="text-base font-bold text-slate-800 mb-2">{t("sales.dashboard.charts.conversionDistributionTitle")}</h3>
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardContent className="p-6">
            <h3 className="text-base font-bold text-slate-800 flex items-center mb-5">
              <ListTodo className="w-5 h-5 mr-2 text-brand-500" /> {t("sales.dashboard.tasks.title")}
            </h3>
            {tasks.length === 0 ? (
              <p className="text-sm text-slate-400 py-4 text-center">
                {t("sales.dashboard.tasks.emptyState")}
              </p>
            ) : (
              <div className="space-y-1">
                {tasks.map((task) => {
                  const companyName = task.customers?.company_name || t("sales.dashboard.unknownCustomer")
                  ...
                  return (
                    ...
                    >
                      <button
                        ...
                        title={t("sales.dashboard.tasks.markCompleteTooltip")}
                      >
                      ...
                      <Link href={`/shared/customers/${task.customer_id}`} className="flex-1 min-w-0">
                        <div className="flex items-baseline justify-between gap-2">
                          ...
                          <span className={`text-xs shrink-0 font-medium ${overdue ? 'text-red-500' : 'text-slate-400'}`}>
                            {overdue ? `${t("sales.dashboard.tasks.overduePrefix")} ` : ''}{formatTaskDueDate(task.due_date, locale)}
                          </span>
                        </div>
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardContent className="p-6">
            <h3 className="text-base font-bold text-slate-800 flex items-center mb-5">
              <History className="w-5 h-5 mr-2 text-brand-500" /> {t("sales.dashboard.activities.title")}
            </h3>
            {recentActivities.length === 0 ? (
              <p className="text-sm text-slate-400 py-4 text-center">
                {t("sales.dashboard.activities.emptyState")}
              </p>
            ) : (
              <div className="space-y-1">
                {recentActivities.map((activity) => {
                  const ActivityIcon = ACTIVITY_TYPE_ICON[activity.type] || ACTIVITY_TYPE_ICON.other
                  const companyName = activity.customers?.company_name || t("sales.dashboard.unknownCustomer")
                  return (
                    <Link
                      key={activity.id}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-baseline justify-between gap-2">
                          ...
                          <span className="text-xs text-slate-400 shrink-0">{formatActivityDateTime(activity.activity_date, locale)}</span>
                        </div>
        ...
        {role === 'admin' && leaderboard.length > 0 && (
          <Card className="border-slate-200 shadow-sm">
            <CardContent className="p-6">
              <h3 className="text-base font-bold text-slate-800 flex items-center mb-5">
                <Trophy className="w-5 h-5 mr-2 text-amber-500" /> {t("sales.dashboard.leaderboard.title")}
              </h3>
              <div className="space-y-1">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-baseline mb-1">
                          ...
                          <span className="text-xs text-slate-500 font-medium shrink-0 ml-2">
                            {rep.total} {t("sales.dashboard.leaderboard.quotesSuffix")} · %{rate} {t("sales.dashboard.leaderboard.approvalSuffix")}
                          </span>
                        </div>
        ...
        {topCustomers.length > 0 && (
          <Card className="border-slate-200 shadow-sm">
            <CardContent className="p-6">
              <h3 className="text-base font-bold text-slate-800 flex items-center mb-5">
                <Crown className="w-5 h-5 mr-2 text-amber-500" /> {t("sales.dashboard.topCustomers.title")}
              </h3>
              <div className="space-y-1">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-baseline mb-1">
                          ...
                          <span className="text-xs text-slate-500 font-medium shrink-0 ml-2">
                            {valueStr} · {c.quoteCount} {t("sales.dashboard.leaderboard.quotesSuffix")}
                          </span>
                        </div>
                        <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                          <div
        ...
        {role === 'admin' && repsQuotaComputed.length > 0 && (
          <div className="mt-6">
            <Card className="border-slate-200 shadow-sm">
              <CardContent className="p-6">
                <h3 className="text-base font-bold text-slate-800 flex items-center mb-5">
                  <Target className="w-5 h-5 mr-2 text-brand-500" /> {t("sales.dashboard.quotaRealization.title")}
                </h3>
                <div className="space-y-1">
