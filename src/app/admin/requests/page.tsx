  ...
  if (requestsWithNames.length > 0) {
    ...
    if (userIds.length > 0) {
      ...
      requestsWithNames = requestsWithNames.map(r => ({
        ...r,
        requester_name: r.requested_by ? (profileMap.get(r.requested_by) || 'Bilinmeyen') : 'Sistem',
      }))
    }
  }
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between h-auto py-5 sm:h-20 sm:py-0 gap-4">
            <div>
              <h1 className="text-2xl font-bold text-slate-900 tracking-tight flex items-center">
                <Inbox className="h-6 w-6 text-brand-600 mr-2" /> Gelen Talepler
              </h1>
              <p className="text-sm text-slate-500 mt-1">Satış personelinden gelen ürün ve envanter güncelleme talepleri.</p>
            </div>
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-1.5 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                <div className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" />
                <span className="text-xs font-bold text-amber-700">{pendingCount} Bekleyen</span>
              </div>
              <div className="flex items-center gap-1.5 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2">
                <span className="text-xs font-bold text-emerald-700">{approvedCount} İşleme Alındı</span>
              </div>
              <div className="flex items-center gap-1.5 bg-red-50 border border-red-200 rounded-lg px-3 py-2">
                <span className="text-xs font-bold text-red-700">{rejectedCount} Reddedildi</span>
              </div>
            </div>
          </div>
        </div>
